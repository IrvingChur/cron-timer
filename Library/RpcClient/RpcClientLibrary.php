...
class RpcClientLibrary
{
    ...
    {
        // 复用连接
        if ($this->objClientInstance instanceof \Swoole\Client) {
            if ($this->objClientInstance->isConnected()) {
                return $this->objClientInstance;
            }
            $this->objClientInstance->close();
            unset($this->objClientInstance);
        }
        ...
        if (!$objClient->isConnected()) {
            throw new \Exception("RPC连接异常，异常代码" . $objClient->errCode);
        }

        $this->objClientInstance = $objClient;
        return $objClient;
    }
}